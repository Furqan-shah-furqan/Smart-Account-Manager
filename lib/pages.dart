    ...
    return Column(
      ...
      children: [
        ...
        const DataCard(
          title: 'Build Status',
          child: Text(
            'Current Supabase version target: 55%. Completed: cloud login, company setup, CRUD, DSR flow, sales, recovery, stock, reports base. Remaining: PDF, Excel, print, subscriptions, audit logs.',
          ...
          SizedBox(
            ...
            child: Column(
              ...
              children: [
                ...
                const SizedBox(height: 8),
                const Text('Supplier → DSR/Booker → Shopkeeper → Cash/Credit Sale → Recovery → Daily DSR Cash Report.', style: TextStyle(color: Colors.white, fontSize: 15, height: 1.5)),
                const SizedBox(height: 18),
                Wrap(
                  spacing: 10,
                  children: [
      ...
      child: state.profile == null
          ...
          : Column(
              ...
              children: [
                ...
                const Text(
                  'Company setup is complete. Now add your real supplier, DSR/booker, shopkeepers, products, and purchase stock manually.',
...
class DsrPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return tablePage(
      title: 'DSR / Booker',
      subtitle: 'Manage bookers, routes, suppliers, salary, and stock.',
      buttonText: 'Add DSR',
...
class SupplierPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return tablePage(
      title: 'Suppliers',
      subtitle: 'Supplier provides goods/products. Linked DSR names are shown before delete.',
                  ...
                  onDelete: () {
                    confirmDelete(
                      ...
                      message: linkedDsrsText == '-'
                          ...
                          : 'This supplier is linked with DSR: $linkedDsrsText. You cannot delete it until you remove or change that DSR supplier.',
    ...
    return Column(
      children: [
        moduleHeader(
          ...
          subtitle: 'Manage products, prices, stock, and low stock limit.',
...
class CompanyPurchasePage extends StatelessWidget {
  ...
  const CompanyPurchasePage({super.key, required this.state, required this.onChanged});
    ...
    return Column(
      children: [
        moduleHeader(
          ...
          subtitle: 'Record stock received from company. App adds packets to warehouse and tracks remaining payable.',
        ...
        const SizedBox(height: 18),
        companyPurchaseTable(state),
      ],
    );
  }
}

class LoadFormPage extends StatelessWidget {
  final AppState state;
  final Future<void> Function() onChanged;
  ...
  Widget build(BuildContext context) {
    return tablePage(
      title: 'Load Form',
      subtitle: 'Load warehouse stock to DSR.',
      buttonText: 'Add Load',
      icon: Icons.move_down_rounded,
      onTap: () => showLoadDialog(context, state, onChanged),
      table: DataTable(
        columns: const [
...
class _ReportDetailPageState extends State<ReportDetailPage> {
  ...
  Widget groupedDsrSalesReport() {
    ...
    return reportTable(
      ...
      headers: const ['DSR', 'Route', 'Bills', 'Cash Sale', 'Credit Sale', 'Total Sale'],
      rows: rows,
    );
  }
  ...
  Widget shopkeeperCreditReport() {
    ...
    return reportTable(
      ...
      headers: const ['Shop', 'Owner', 'DSR', 'Credit Added', 'Paid', 'Filter Balance', 'Current Balance'],
      rows: rows,
    );
  }
...
Widget companyPurchaseTable(AppState state) {
  return DataCard(
    title: 'Company Purchase History',
        ...
        : horizontalTable(
            DataTable(
              ...
              columns: const [
                ...
                DataColumn(label: Text('Product')),
                DataColumn(label: Text('Batch')),
                DataColumn(label: Text('Cartons')),
                DataColumn(label: Text('Packets/Carton')),
                DataColumn(label: Text('Total Packets')),
                DataColumn(label: Text('Packet Price')),
              ...
              rows: state.companyPurchases.map((x) {
                return DataRow(cells: [
                  ...
                  DataCell(Text(x.batchNo.isEmpty ? '-' : x.batchNo)),
                  DataCell(Text(x.cartons.toString())),
                  DataCell(Text(x.packetsPerCarton.toString())),
                  DataCell(Text(x.totalPackets.toString())),
                  DataCell(Text(state.rs(x.packetPurchasePrice))),
                  DataCell(Text(state.rs(x.companyDiscount))),
                  DataCell(Text(state.rs(x.totalBill))),
                  DataCell(Text(state.rs(x.paidAmount))),
                  DataCell(Text(state.rs(x.remainingAmount))),
                  DataCell(SizedBox(width: 160, child: Text(x.note.isEmpty ? '-' : x.note, overflow: TextOverflow.ellipsis))),
                ]);
              }).toList(),
            ),
          ),
...
Widget salesTable(AppState state, {List<SaleEntry>? rows}) {
  ...
  Widget compactText(String value, double width, {TextAlign align = TextAlign.left}) {
    return SizedBox(
      width: width,
      child: Text(
        value,
                ...
                child: ConstrainedBox(
                  ...
                  child: DataTable(
                    ...
                    rows: data.map((x) {
                      return DataRow(
                        cells: [
                          DataCell(compactText(x.billNo, 82)),
                          DataCell(compactText(formatDateForUi(x.date), 86)),
                          DataCell(compactText(state.dsrName(x.dsrId), 110)),
                          DataCell(compactText(state.shopName(x.shopkeeperId), 140)),
                          DataCell(compactText(state.productName(x.productId), 140)),
                          DataCell(compactText(x.quantity.toString(), 42, align: TextAlign.center)),
                          DataCell(compactText(state.rs(x.price), 78)),
                          DataCell(compactText(x.type == SaleType.cash ? 'Cash' : 'Credit', 58)),
                          DataCell(compactText(state.rs(x.total), 86)),
                        ],
                      );
                    }).toList(),
                  ),
...
void showSupplierDialog(BuildContext context, AppState state, Future<void> Function() onChanged, {Supplier? editItem}) {
  ...
  simpleDialog(
    context: context,
    title: editItem == null ? 'Add Supplier' : 'Edit Supplier',
    children: [
      textInput(label: 'Supplier Name', controller: nameController),
...
void showDsrDialog(BuildContext context, AppState state, Future<void> Function() onChanged, {Dsr? editItem}) {
  if (state.suppliers.isEmpty) {
    showSnack(context, 'Add supplier first.');
    return;
  }
    ...
    builder: (setDialog) {
      return [
        ...
        DropdownButtonFormField<String>(
          value: supplierId,
          decoration: const InputDecoration(labelText: 'Assigned Supplier'),
...
void showProductDialog(BuildContext context, AppState state, Future<void> Function() onChanged, {Product? editItem}) {
  ...
  simpleDialog(
    context: context,
    title: editItem == null ? 'Add Product' : 'Edit Product',
    children: [
      const Align(
...
void showLoadDialog(BuildContext context, AppState state, Future<void> Function() onChanged) {
  if (state.dsrs.isEmpty || state.suppliers.isEmpty || state.products.isEmpty) {
    showSnack(context, 'Add DSR, supplier, and product first.');
    return;
  }
  ...
  statefulDialog(
    context: context,
    title: 'Load Stock to DSR',
    builder: (setDialog) {
      return [
        DropdownButtonFormField<String>(
          value: dsrId,
          decoration: const InputDecoration(labelText: 'DSR'),
          items: state.dsrs.map((x) => DropdownMenuItem(value: x.id, child: Text(x.name))).toList(),
          onChanged: (value) {
            setDialog(() {
              dsrId = value ?? dsrId;
        ...
        textInput(label: 'Load Quantity', controller: qtyController, number: true),
      ];
    },
    onSave: () async {
      await runAction(
